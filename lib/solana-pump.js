...
export function isSolanaSignature(value) {
  ...
  if (signature.length < 64 || signature.length > 88 || !/^[1-9A-HJ-NP-Za-km-z]+$/.test(signature)) return false;
  let bytes = [0];
  ...
  for (const character of signature) { if (character === "1") bytes.push(0); else break; }
...
export async function refreshPumpTarget() {
  ...
  await Promise.all([
    ...
    supabaseRequest("/o8_system_bindings?id=eq.pump-fun", { method: "PATCH", body: JSON.stringify({ status: "PUMP_UNVERIFIED", truth: "CONNECTED", details: { platform: "PUMP.FUN", targetMint: mint, verification: metadata.verification }, updated_at: new Date().toISOString() }) }),
  ]);
