...
export async function refreshPumpTarget() {
  ...
  if (!isSolanaAddress(target.token_address)) {
    await Promise.all([
      supabaseRequest("/o8_token_targets?id=eq.o8", { method: "PATCH", body: JSON.stringify({ platform: "PUMP.FUN", symbol: "COMB", token_name: null, token_address: null, launch_tx_hash: null, launch_block: null, launched_at: null, status: "AWAITING_PUMP_CA", metadata: { chain: "SOLANA", launchpad: "PUMP.FUN", verification: "TARGET_MINT_REQUIRED" }, updated_at: new Date().toISOString() }) }),
      supabaseRequest("/o8_system_bindings?id=eq.o8-token", { method: "PATCH", body: JSON.stringify({ status: "AWAITING_PUMP_CA", details: { symbol: "COMB", contractAddress: null, launchpad: "PUMP.FUN", stage: "AWAITING_PUMP_CA" }, updated_at: new Date().toISOString() }) }),
    ]);
  ...
  await Promise.all([
    ...
    supabaseRequest("/o8_system_bindings?id=eq.o8-token", { method: "PATCH", body: JSON.stringify({ status: "PUMP_ACTIVE", truth: "CONNECTED", details: { symbol: "COMB", contractAddress: mint, launchpad: "PUMP.FUN", stage: "PUMP_ACTIVE", verification: metadata.verification, explorer: metadata.explorer }, updated_at: new Date().toISOString() }) }),
  ]);
