...
async function retainHead(head) {
  ...
  await Promise.all([
    ...
    supabaseRequest("/o8_system_bindings?on_conflict=id", { method: "POST", headers: { Prefer: "resolution=merge-duplicates" }, body: JSON.stringify({ id: "pump-fun", label: "Pump.fun target lifecycle", status: "AWAITING_PUMP_CA", truth: "CONNECTED", details: { platform: "PUMP.FUN", verification: "TARGET_MINT_REQUIRED" }, updated_at: new Date().toISOString() }) }),
